...
import {Fib, Iter} from "./iterator.js";

const suite = TestSuite("iterator");

suite.add("ctor", assert => {
    assert.iterableEq(Iter(0, i=> i < 5, i=> i + 1), [0,1,2,3,4]);
    assert.iterableEq(Iter(0, i=> i < 0, i=> i + 1), []);
    assert.iterableEq(Iter(0, i=> i < 1, i=> i + 1), [0]);
});
...
suite.add("infinite", assert => {
    const numbers = Iter(0, _i => true, i => i + 1);
